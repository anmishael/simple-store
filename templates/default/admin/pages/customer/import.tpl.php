...
<h2><?php echo $this->_core->getModel('Translate/Collection')->get('ADMIN_TEXT_IMPORT_CUSTOMER', 'value')->get('value'); ?></h2>
<form id="admin-customer-import-form" action="<?php echo $this->_core->getActionUrl('admin-customer-import');?>" method="post" enctype="multipart/form-data">
	<table>
		<tr>
...
<table>
	...
	<tr>
		<td><a rel="<?php echo $this->_tmpFile; ?>" class="import-file-link" href="?path=<?php echo $item['path']; ?>"><?php echo $item['name']; ?></a></td>
...
<script type="text/javascript">
	$(document).ready(function() {
		$('.button-cancel').bind('click', function() {
			window.location = '<?php echo addslashes($this->_core->getActionUrl('admin-customer-list') . '?' . $this->_core->getAllGetParams(array('id'))); ?>';
		});
	});
