...
?>
<h1><?php echo $this->_core->getModel('Translate/Collection')->get('CART_PRODUCT_BOX_TITLE')->get('value'); ?></h1>
<hr />
<div id="block-cart-list" class="product-list" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html">
	...
	<table cellspacing="0" cellpadding="10" width="100%" class="bundleproducts">
		<tr>
			<th class="first"> &nbsp; </th>
	...
	<?php foreach($this->_items as $k=>$item) { ?>
		...
		<tr>
			<td class="remove first"><a class="remove" href="<?php echo $this->topFolder; ?>cart/remove?id=<?php echo $item->get('id'); ?>" title="remove"><span class="title">remove</span></a></td>
			...
			<td class="nowrap last"><?php echo number_format($item->get('qty')*$item->getPrice(), 2, '.', ' ') . ' ' . $this->_currency->get('label'); ?></td>
		</tr>
	<?php } ?>
	</table>
	<div class="total">
		<div class="summ"><?php echo $this->_core->getModel('Translate/Collection')->get('CART_PRODUCT_TOTAL')->get('value') . ' <span  class="summ-numeric">' . $this->_cart->subtotalsumm() . ' ' . $this->_currency->get('label'); ?></span></div>
		<div class="clear"></div>
	</div>
	<div class="buttons">
